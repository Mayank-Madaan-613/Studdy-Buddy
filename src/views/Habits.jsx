...
const HabitsView = () => {
  ...
  const toggleHistory = (habitId, dayIndex) => {
    setHabits(habits.map(h => {
      if (h.id === habitId) {
        ...
        // Calculate current streak based on consecutive completed days (backwards from today or simply total completed)
        // Here we do a simple filter(Boolean) length for total completed, matching prototype logic
  ...
  const addHabit = () => {
    ...
    if (name) {
      const category = prompt("Enter category (Learning, Health, Wellness, Study):", "Learning");
      setHabits([
        ...habits, 
        { 
          id: Date.now(), 
  ...
  return (
    <div className="space-y-8 pt-4 pb-10 max-w-5xl mx-auto">
      <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
        ...
        <Button onClick={addHabit} className="rounded-xl px-5">
          <Icons.Plus className="w-5 h-5 mr-2" /> New Habit
        </Button>
      </header>
            ...
            <Card className="border border-slate-700/50 hover:border-slate-500/50 transition-all flex flex-col lg:flex-row gap-6 lg:items-center justify-between bg-slate-800/20">
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-2">
                  <h3 className="text-xl font-bold text-white max-w-[150px] sm:max-w-[200px] truncate" title={habit.name}>
                    {habit.name}
                  </h3>
                  ...
                  <button 
                    ...
                    className="ml-auto lg:ml-2 text-slate-500 hover:text-rose-500 transition-colors p-1"
                    title="Delete Habit"
                  >
              ...
              <div className="flex gap-2.5 sm:gap-4 overflow-x-auto pb-2 sm:pb-0 no-scrollbar">
                {habit.history.map((status, i) => (
                  <div key={i} className="flex flex-col items-center gap-2">
                    ...
                    <button 
                      onClick={() => toggleHistory(habit.id, i)}
                      className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all duration-300 hover:scale-105 active:scale-95 cursor-pointer ${
                        status 
                          ? 'bg-gradient-to-br from-emerald-400 to-teal-500 text-white shadow-[0_4px_15px_-3px_rgba(16,185,129,0.3)] border-none shrink-0' 
                          : 'bg-slate-800/80 border border-slate-700 text-transparent hover:border-slate-500 shrink-0'
                      }`}
                    >
