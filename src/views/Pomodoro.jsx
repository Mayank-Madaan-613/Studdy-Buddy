    ...
    <div className="flex flex-col items-center justify-center min-h-[85vh] py-6 w-full max-w-3xl mx-auto">
      ...
      >
        <div className="flex gap-2 p-1.5 bg-slate-800/80 backdrop-blur-md rounded-2xl border border-slate-700/80 shadow-xl">
          <button 
            ...
            className={`px-6 py-2.5 rounded-xl text-sm font-semibold transition-all duration-300 ${mode === 'focus' ? 'bg-primary text-white shadow-lg shadow-primary/30' : 'text-slate-400 hover:text-white hover:bg-slate-700/50'}`}
          >
            Focus (25m)
          </button>
          <button 
            onClick={() => handleMode('shortBreak')} 
            className={`px-6 py-2.5 rounded-xl text-sm font-semibold transition-all duration-300 ${mode === 'shortBreak' ? 'bg-secondary text-white shadow-lg shadow-secondary/30' : 'text-slate-400 hover:text-white hover:bg-slate-700/50'}`}
          >
            Short Break (5m)
          </button>
          <button 
            onClick={() => handleMode('longBreak')} 
            className={`px-6 py-2.5 rounded-xl text-sm font-semibold transition-all duration-300 ${mode === 'longBreak' ? 'bg-indigo-500 text-white shadow-lg shadow-indigo-500/30' : 'text-slate-400 hover:text-white hover:bg-slate-700/50'}`}
          >
            Long Break (15m)
          </button>
        </div>
      ...
      <Card className="w-full flex flex-col items-center py-12 px-6 border-slate-700/50 bg-slate-900/50 relative overflow-hidden">
        ...
        <div className="flex items-center gap-8">
          <button 
            onClick={resetTimer} 
            className="p-4 rounded-2xl bg-slate-800 hover:bg-slate-700 text-slate-300 transition-colors border border-slate-700 shadow-md"
            title="Reset Timer"
          >
          ...
          <button 
            onClick={toggleTimer} 
            className={`w-24 h-24 rounded-3xl flex items-center justify-center transition-all duration-300 ${isActive ? 'bg-rose-500 hover:bg-rose-600 shadow-rose-500/30' : mode === 'focus' ? 'bg-primary hover:bg-indigo-500 shadow-primary/30' : 'bg-secondary hover:bg-teal-500 shadow-secondary/30'} shadow-xl group`}
          ...
          <button 
            onClick={addTime} 
            className="p-4 rounded-2xl bg-slate-800 hover:bg-slate-700 text-slate-300 transition-colors border border-slate-700 shadow-md active:bg-slate-600 active:scale-95"
            title="Add 5 Minutes"
          >
